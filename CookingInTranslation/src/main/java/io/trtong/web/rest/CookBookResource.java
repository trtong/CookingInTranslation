...
public class CookBookResource {
    ...
    @Timed
    public List<CookBook> getAllCookBooks(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get all CookBooks");
        return cookBookRepository.findAllWithEagerRelationships();
    }
    ...
    public ResponseEntity<CookBook> getCookBook(@PathVariable Long id) {
        ...
        Optional<CookBook> cookBook = cookBookRepository.findOneWithEagerRelationships(id);

...
public class CookBook implements Serializable {
    ...
    @ManyToMany
    @Cache(usage = CacheConcurrencyStrategy.NONSTRICT_READ_WRITE)
    @JoinTable(name = "cook_book_all_recipes",
               joinColumns = @JoinColumn(name = "cook_books_id", referencedColumnName = "id"),
               inverseJoinColumns = @JoinColumn(name = "all_recipes_id", referencedColumnName = "id"))
    private Set<Recipe> allRecipes = new HashSet<>();

    @ManyToMany
    @Cache(usage = CacheConcurrencyStrategy.NONSTRICT_READ_WRITE)
    @JoinTable(name = "cook_book_translated_recipes",
               joinColumns = @JoinColumn(name = "cook_books_id", referencedColumnName = "id"),
               inverseJoinColumns = @JoinColumn(name = "translated_recipes_id", referencedColumnName = "id"))
    ...
    public CookBook allRecipes(Set<Recipe> recipes) {
        this.allRecipes = recipes;
        return this;
    }

    public CookBook addAllRecipes(Recipe recipe) {
        this.allRecipes.add(recipe);
        recipe.getCookbooks().add(this);
        return this;
    }

    public CookBook removeAllRecipes(Recipe recipe) {
        this.allRecipes.remove(recipe);
        recipe.getCookbooks().remove(this);
        return this;
    }

    public void setAllRecipes(Set<Recipe> recipes) {
        this.allRecipes = recipes;
    }

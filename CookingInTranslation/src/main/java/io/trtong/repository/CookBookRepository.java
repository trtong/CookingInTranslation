...
public interface CookBookRepository extends JpaRepository<CookBook, Long> {
    ...
    @Query(value = "select distinct cook_book from CookBook cook_book left join fetch cook_book.recipes left join fetch cook_book.translatedRecipes",
    ...
    @Query("select cook_book from CookBook cook_book left join fetch cook_book.recipes left join fetch cook_book.translatedRecipes where cook_book.id =:id")

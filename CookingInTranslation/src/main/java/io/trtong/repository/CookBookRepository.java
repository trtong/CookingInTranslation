...
public interface CookBookRepository extends JpaRepository<CookBook, Long> {
    ...
    @Query(value = "select distinct cook_book from CookBook cook_book left join fetch cook_book.recipes",
    ...
    @Query("select cook_book from CookBook cook_book left join fetch cook_book.recipes where cook_book.id =:id")
